...
def _recheck_handle(handle: VerifiedControlHandle, context: Stage51PhysicsContext) -> None:
    ...
    expected_files = [{"path": row["path"], "byte_length": row["byte_length"], "raw_sha256": row["raw_sha256"]} for row in actual if row.get("entry_type") == "file" and row["path"] not in {MANIFEST_NAME, REPORT_NAME, RECEIPT_NAME}]
