...
def install_synthetic_spectroscopy_runner(monkeypatch, calls, *, cross_excitation: float = 0.005) -> None:
    ...
    def fake_run(circuits, _context_value, output_root, _repository_root, **kwargs):
        ...
        for circuit in circuits:
            ...
            results.append(replace(result, evidence_root=circuit_evidence, model_evidence_root=model_evidence))
...
def install_evidence_runner(monkeypatch) -> None:
    def fake_run(circuits, _context_value, output_root, _repository_root, **_kwargs):
        ...
        for circuit in circuits:
            ...
            results.append(replace(spectroscopy_result(circuit.circuit_id, 0.8, 0.19, 0.0, 0.0), evidence_root=evidence_root, model_evidence_root=evidence_root))
        return tuple(results)
    monkeypatch.setattr(spectroscopy_module, "run_circuits", fake_run)
